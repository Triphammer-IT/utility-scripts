...
handle_batch_operations() {
    ...
    echo "3) Process only repositories with UNPUSHED commits"
    echo "4) Skip batch operations"
    ...
    case $choice in
        ...
        2)
            ...
            echo "Enter repository numbers (space-separated), ranges (e.g., 3-7), or 'all' for all:"
                    ...
                    if [[ "$item" =~ ^([0-9]+)-([0-9]+)$ ]]; then
                        ...
                        for ((num=start; num<=end; num++)); do
                            if [[ $num -ge 1 && $num -le ${#all_repos[@]} ]]; then
                                selected_repos+=("${all_repos[$((num-1))]}")
                            fi
                        done
                    elif [[ "$item" =~ ^[0-9]+$ ]]; then
            ...
            ;;
        3)
            print_status "$GREEN" "Processing repositories with unpushed commits..."
            local unpushed_repos=()

            # Combine repos with unpushed commits
            if [[ ${#repos_with_both[@]} -gt 0 ]]; then
                unpushed_repos+=("${repos_with_both[@]}")
            fi
            if [[ ${#repos_with_unpushed[@]} -gt 0 ]]; then
                unpushed_repos+=("${repos_with_unpushed[@]}")
            fi
...
show_help() {
    ...
    echo "  - Option 1: Process all repositories with changes"
    echo "  - Option 2: Select specific repositories (supports ranges like 3-7)"
    echo "  - Option 3: Process only repositories with unpushed commits"
    echo "  - Option 4: Skip batch operations"
}
